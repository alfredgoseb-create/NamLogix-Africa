    ...
    <div className="container">
      <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '40px' }}>
        <div>
          ...
          <p style={{ color: '#64748b', marginTop: '5px' }}>Live Inventory Management</p>
        </div>
            ...
            {inventory.map((item) => (
              <tr key={item.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                ...
                <td style={{ padding: '20px', textAlign: 'center' }}>
                  <span style={{ padding: '6px 16px', background: item.quantity < 5 ? '#fee2e2' : '#dcfce7', color: item.quantity < 5 ? '#991b1b' : '#166534', borderRadius: '20px', fontWeight: 'bold' }}>
                    {item.quantity} units
                  </span>
                </td>
