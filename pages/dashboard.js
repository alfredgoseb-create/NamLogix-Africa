      ...
      <div style={{ maxWidth: '1000px', margin: '0 auto' }}>
        ...
        <div style={{ backgroundColor: 'white', borderRadius: '12px', overflow: 'hidden', boxShadow: '0 4px 6px rgba(0,0,0,0.05)' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan="4" style={{ padding: '20px', textAlign: 'center' }}>Loading stock...</td></tr>
              ) : products.length === 0 ? (
                <tr><td colSpan="4" style={{ padding: '20px', textAlign: 'center' }}>No inventory found. Click "+ New Stock" to begin.</td></tr>
              ) : (
                products.map((item) => (
                  <tr key={item.id} style={{ borderBottom: '1px solid #f3f4f6' }}>
                    <td style={{ padding: '15px', fontWeight: '500' }}>{item.name}</td>
                    <td style={{ padding: '15px' }}>{item.stock_level} units</td>
                    <td style={{ padding: '15px', color: '#6b7280' }}>{item.supplier || 'N/A'}</td>
