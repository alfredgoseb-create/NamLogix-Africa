...
export default function SuppliersPage() {
  ...
  const [form, setForm] = useState({ name: "", contact_name: "", email: "", phone: "", category: "" });
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    setForm({ name: "", contact_name: "", email: "", phone: "", category: "" });
    setEditingId(null);
    fetchSuppliers();
  }
  ...
  function handleEdit(s: Supplier) {
    setForm({ name: s.name, contact_name: s.contact_name, email: s.email, phone: s.phone, category: s.category });
    setEditingId(s.id);
    window.scrollTo(0, 0);
  }
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-5xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          ...
          <button onClick={() => router.push("/admin/dashboard")} className="px-4 py-2 bg-gray-100 rounded-lg text-sm">← Dashboard</button>
        </div>
        <div className="bg-white rounded-xl shadow p-6 mb-8">
          ...
          <form onSubmit={handleSave} className="grid md:grid-cols-2 gap-4">
            ...
            <div className="col-span-2 flex gap-3">
              ...
              {editingId && <button type="button" onClick={() => { setEditingId(null); setForm({ name: "", contact_name: "", email: "", phone: "", category: "" }); }} className="px-6 py-2 rounded-lg bg-gray-100">Cancel</button>}
            </div>
          </form>
        </div>
              ...
              {suppliers.map((s) => (
                <div key={s.id} className="border rounded-xl p-4 flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                  <div>
                    ...
                    {s.category && <span className="text-xs bg-blue-50 text-blue-600 px-2 py-1 rounded-full mt-1 inline-block">{s.category}</span>}
                  </div>
